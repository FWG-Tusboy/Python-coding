...
class UpdateTracks:
    ...
    def update_track(self):
        ...
        if not track_key or not new_rating:
            # if input is not put in completely, status will remind for user to input in both fields
        ...
        self.status_lbl.config(
            text=f"Rating has been changed at {track_key} ({name}) to {rating_value} stars."
        )
